...
past = load(['C:\Users\bryan\Documents\GitHub\MatrixMultiplication\Experiments\data\moARC_' num2str(r) '.mat']);
x0=past.x;
x=zeros(size(x0));
errorHistory1 = zeros(options.maxiter,N);

for i=1:N
    if past.errorHistory(end) < options.tolcost
        continue
    end
    X0 = mat(x0(:,i));
    [X, ~, info, ~] = arc(problem,X0,options);
    ...
    iter=[info.iter];
    cost=[info.cost];
    errorHistory1(1:iter(end),i) =  cost(2:end).';
end

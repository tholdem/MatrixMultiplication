                   ...
                   maxIts, emxArray_real_T *p)
{
  ...
  do {
    ...
    if ((fabs(normp) > 0.001) && (i < maxIts)) {
      ...
      k1 = r5->size[0] * r5->size[1];
      r5->size[0] = 1;
      companDim = div_s32_floor(j, nTrailingZeros);
      r5->size[1] = companDim + 1;
      emxEnsureCapacity_int32_T(r5, k1);
      for (j = 0; j <= companDim; j++) {
        r5->data[j] = nTrailingZeros * j;
      }

      c_diag(B, q);
      companDim = r5->size[0] * r5->size[1] - 1;
      for (j = 0; j <= companDim; j++) {
        B1->data[r5->data[j]] = q->data[j] + lambda;
      }
      ...
      if (info != 0) {
        ...
        for (j = 0; j <= companDim; j++) {
          B1->data[r5->data[j]] = q->data[j] + normp;
        }
      ...
      if (guard2) {
        ...
        if (normp <= Delta) {
          if ((lambda == 0.0) || (normp == Delta)) {
            ...
            j = r6->size[0] * r6->size[1];
            r6->size[0] = v1->size[0];
            r6->size[1] = v1->size[1];
            emxEnsureCapacity_real_T(r6, j);
            companDim = v1->size[0] * v1->size[1];
            for (j = 0; j < companDim; j++) {
              r6->data[j] = a * v1->data[j];
            }
